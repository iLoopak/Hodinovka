...
- [x] Wrap the existing PWA in Capacitor — `capacitor.config.ts` points `webDir` at the Next.js static export (`out/`); appId `com.iloopak.hodinovka`, appName "Hodinovka". Android platform scaffolded in `android/` with the Filesystem + Share plugins.
- [x] Share/save without the Web APIs in the WebView — `lib/native.ts` detects the native shell (`Capacitor.isNativePlatform()`) and routes **Sdílet** through the Capacitor **Share** plugin (writes the PDF to cache, shares its URI) and **Stáhnout / ISDOC** through **Filesystem** (writes to Documents). Web/PWA keeps the existing `navigator.share()` + `<a download>` + `mailto:` fallback untouched. Plugins load dynamically so they stay out of the web bundle.
- [ ] On-device verification: install the debug APK on a real Android device and confirm (a) the share sheet attaches the PDF, (b) logo/PDF Blobs survive in IndexedDB, and (c) storage limits are fine for typical use. *(Needs a device/emulator; not doable in CI.)*

> **Build:** see [`docs/android-build.md`](./android-build.md). `npm run android:sync` rebuilds the export and copies it into the native project; `npm run android:apk` also runs Gradle `assembleDebug` → `android/app/build/outputs/apk/debug/app-debug.apk`. Requires a JDK (21 works) and the Android SDK (`ANDROID_HOME` or `android/local.properties` → `sdk.dir`). The `android/` project is committed; copied web assets, generated config, `local.properties`, and Gradle build output are git-ignored (regenerated by `cap sync`).
